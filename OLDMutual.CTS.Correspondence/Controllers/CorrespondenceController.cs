...
namespace OLDMutual.CTS.Correspondence.Controllers
{
    ...
    public class CorrespondenceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDropdowndata(string type)
        {
            try
            {
                ...
                {
                    var result = await _CorrespondenceService.GETDropDownData(type);
                    if (!result.Any())
                    {
        ...
        [Authorize]
        public async Task<IActionResult> PromoteSubmissionFile(int file_id)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.PromoteCorrespondence(file_id);
        ...
        public async Task<IActionResult> SubmitSARSFile(int file_id)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.SubmitSARSFile(file_id);
        ...
        [Authorize]
        public async Task<IActionResult> DeletSubmissionFile(List<FileModel> CorrespondenceData)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.DeleteCorrespondence(CorrespondenceData);
        ...
        [Authorize]
        public async Task<IActionResult> DeleteErrorRecord(List<FileModel> CorrespondenceErrorData, string fileType)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.DeleteErrorRecord(CorrespondenceErrorData, fileType);
        ...
        public async Task<IActionResult> GenerateSnapshot(int file_id, int fileRegionId)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.GenerateSnapshot(file_id, fileRegionId);
        ...
        public async Task<IActionResult> GetActionRules()
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.GetActionRules();
                    if (result == null)
                    {
                        js.Statuscode = (int)HttpStatusCode.NotFound;
        ...
        /// <returns></returns>
        [HttpGet("GetCorrespondenceErrorData")]
        [Authorize]
        public async Task<IActionResult> ViewErrorReport(int page, int size, int file_Id)
        {
            try
            {
                if (HttpContext.User.Identity.IsAuthenticated)
                {
                    var result = await _CorrespondenceService.ViewErrorReport(page, size, file_Id);
                    if (result == null)
                    {
                        js.Statuscode = (int)HttpStatusCode.NotFound;
                        js.Message = "No data found";
